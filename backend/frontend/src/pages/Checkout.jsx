...
import { switzerlandTours as switzerlandToursData, normalizeSwissId } from "../data/switzerlandTours";
import { tourDescriptions } from "../data/tourDescriptions";

const switzerlandTours = switzerlandToursData;

const getStaticSwitzerlandTour = (id) => {
  const normalizedId = normalizeSwissId(id);
  const staticTour = switzerlandTours.find((item) => item.id === normalizedId);
  ...
  return normalizeTourData({
    ...
    description: staticTour.desc || staticTour.description || '',
    overview: staticTour.desc || staticTour.description || '',
    startLocation: staticTour.address || '',
    endLocation: staticTour.address || '',
    tourType: 'Day Tour, Private Tour',
    reviewText:
      staticTour.rating || staticTour.reviews
        ? `${staticTour.rating || 0} (${staticTour.reviews || 0} reviews)`
        : 'No reviews yet',
    included: staticTour.features || [],
    excluded: staticTour.excluded || [],
    itinerary: Array.isArray(staticTour.itinerary)
      ? staticTour.itinerary.map((item) =>
          typeof item === 'string'
            ? { title: item, description: '', duration: '', location: '', activities: [] }
            : item
        )
      : [],
  });
  ...
  const saveArrayField = async (fieldName, arrayData) => {
    ...
    try {
      const res = await fetch(`${getBackendUrl()}/api/tours/${tourId}`, {
        ...
        body: JSON.stringify({ ...tour, [fieldName]: arrayData })
      });
      if (res.ok) {
  ...
  const saveSingleField = async (fieldName, value) => {
    ...
    try {
      const res = await fetch(`${getBackendUrl()}/api/tours/${tourId}`, {
        ...
        body: JSON.stringify({ ...tour, [fieldName]: value })
      });
      if (res.ok) {
        const response = await res.json();
        const tourData = response.tour || response;
          ...
          onSave={async (value) => {
            ...
            try {
              const res = await fetch(`${getBackendUrl()}/api/tours/${tourId}`, {
                ...
                body: JSON.stringify({ ...tour, name: value.replace('Explore ', '') })
              });
              if (res.ok) {
        ...
        <div className="flex-1 bg-white rounded-xl p-8 shadow">
                ...
                onSave={async (value) => {
                  ...
                  try {
                    ...
                    const res = await fetch(`${backendUrl}/api/tours/${tourId}`, {
                      ...
                      body: JSON.stringify({ ...tour, name: value })
                    });
                    if (res.ok) {
                      const response = await res.json();
                      const tourData = response.tour || response;
          ...
          <CalendarPrices selectedDate={selectedDate} setSelectedDate={setSelectedDate} />
            ...
            <div className="flex items-center gap-4">
              ...
              />
              <span className="text-red-700 font-semibold">CHF199</span>
            </div>
          )}
        </div>
            ...
            onSaveMinTickets={async (minValue) => {
              ...
              try {
                const res = await fetch(`${getBackendUrl()}/api/tours/${tourId}`, {
                  ...
                  body: JSON.stringify({ ...tour, minTicketsPerBooking: minValue })
                });
                if (res.ok) {
                  const response = await res.json();
                  const tourData = response.tour || response;
