...
const Navbar = () => {
  ...
  const [swissTours] = useState(switzerlandTours || []);
  ...
  useEffect(() => {
    const fetchTours = async () => {
      try {
        const res = await axios.get('/api/tours?view=search&limit=100');
                        ...
                        return (
                          <li key={id}>
                            <button
                              ...
                              onClick={() => {
                                if (id) {
                                  navigate(`/switzerland/${id}/checkout-sw`);
                                } else {
                                  navigate("/switzerland");
                                }
                                setDestinationsOpen(false);
