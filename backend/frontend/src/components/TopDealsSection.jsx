      ...
      try {
        ...
        const toursWithSales = allTours.map(tour => {
          ...
          return {
            ...
            address: tour.startLocation || tour.address || staticTour?.address,
        ...
        if (topTours.length < 3) {
          ...
          setTopSwissTours([...topTours, ...staticToursToAdd]);
        } else {
          setTopSwissTours(topTours);
        }
      } catch (error) {
        console.error('Error loading top selling tours:', error);
        // Fallback to static data sorted by price (as placeholder)
        const fallbackTours = [...switzerlandTours]
          .filter((t) => t && t.name && Array.isArray(t.images) && t.images.length > 0)
          .sort((a, b) => (b.price || 0) - (a.price || 0))
          .slice(0, 3);
        setTopSwissTours(fallbackTours);
      } finally {
        setLoading(false);
      }
    };
