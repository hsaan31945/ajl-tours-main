...
// Prefer backend tours; fallback to static if needed
import { switzerlandTours } from "../data/switzerlandTours";
...
const ExploreTours = () => {
  ...
  const getRandomTours = async () => {
    ...
    const allTours = [...switzerlandTours.slice(0, 12)];
    const validTours = allTours.filter(tour => 
      tour && tour.name && tour.price && Array.isArray(tour.images) && tour.images.length > 0
    );
    const shuffled = validTours.sort(() => 0.5 - Math.random());
    return shuffled.slice(0, 6);
  };
  ...
  useEffect(() => {
    (async () => {
      const tours = await getRandomTours();
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {isMobile ? (
          <div className="space-y-8">
            {randomTours.slice(0, 3).map((tour) => (
              <TourCard
                key={tour.id}
                tour={tour}
        ...
        <div className="text-center mt-6 sm:mt-8">
          <button 
            ...
            className="bg-gray-800 text-white px-4 sm:px-6 lg:px-8 py-2 sm:py-3 rounded-lg font-semibold hover:bg-gray-900 transition-colors duration-300 text-sm sm:text-base"
          >
            Show Different Tours
          </button>
        </div>
      </div>
    </section>
