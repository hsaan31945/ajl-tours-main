  ...
  try {
    ...
    if (result.status === 200) {
      ...
      if (action === 'backup' && result.data.backup) {
        ...
        const filename = path.join(__dirname, `backup_${timestamp}.json`);
...
📋 Database Tools - Usage:
  ...
  node run-database-tools.js backup    - Backup database
  node run-database-tools.js migrate   - Migrate to separate collections
  node run-database-tools.js clear    - Clear database (DANGEROUS!)
